...
class LPIPS(nn.Module):
    ...
    def __init__(self, use_dropout=True):
        ...
        self.scales = [1, 0.5, 0.25, 0.125] # multi-scale perceptual
    ...
    def forward(self, input, target):
        ...
        pyramide_real = self.pyramid(input)   ## real image
        pyramide_generated = self.pyramid(target)  ## prediction image
        for scale in self.scales:
            in0_input = self.scaling_layer(pyramide_generated['prediction_' + str(scale)])
            in1_input = self.scaling_layer(pyramide_real['prediction_' + str(scale)])
